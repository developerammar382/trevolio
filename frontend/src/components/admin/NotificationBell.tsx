...
export default function NotificationBell() {
    ...
    const fetchNotifications = async () => {
        try {
            const response = await api.get('/admin/notifications/recent?group=orders');
    ...
    useEffect(() => {
        try {
            ...
            const handleNewOrder = (e: any) => {
                console.log('🔔 NotificationBell: New order received, refreshing notifications');
                fetchNotifications();
            };

            channel.listen('NewOrderNotification', handleNewOrder);
            channel.listen('.NewOrderNotification', handleNewOrder);
            channel.listen('App\\Events\\NewOrderNotification', handleNewOrder);

            return () => {
                channel.stopListening('NewOrderNotification');
                channel.stopListening('.NewOrderNotification');
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-slate-100 py-2 z-50">
                    <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between">
                        <h3 className="font-semibold text-slate-900">Order Notifications</h3>
                        <Link
                            href="/admin/notifications?type=new_order"
